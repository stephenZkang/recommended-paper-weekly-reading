...
A --> C[研究背景与挑战]
C --> C1[生成式推荐（GR）痛点]
C1 --> C11[单模态局限：文本模态易受品牌特征干扰，语义区分度低（如同一品牌不同乐器表征相似）]
C1 --> C12[跨模态交互缺失：现有方法独立量化多模态，未利用互补信息，码本利用率低]
C1 --> C13[语义损失严重：RQ-VAE深层量化易丢失语义，导致码本分配随机]
C --> C2[核心矛盾]
C2 --> C21[多模态特征互补性 vs 现有方法独立建模]
C2 --> C22[高质量语义ID需求 vs 单模态表征缺陷]

A --> D[MACRec框架设计]

D --> D1[1. 跨模态物品量化模块（生成语义ID）]
D1 --> D11[双模态伪标签生成：文本/图像嵌入K-means聚类（K=512），构建对比学习正样本]
D1 --> D12[跨模态对比量化：每层RQ-VAE引入跨模态对比损失，降低语义损失]
D1 --> D13[跨模态重构对齐：量化后表征双向对齐，提升码本利用率]
D1 --> D14[总损失]
D --> D2[2. 多维度对齐的生成推荐模块]
D2 --> D21[隐式对齐：T5编码器编码多模态语义ID，latent空间对比学习]
D2 --> D22[显式对齐：物品级（文本ID→图像ID/反之）、序列级（历史序列→另一模态下一个ID）生成任务]
D2 --> D23[推荐损失]
...
E --> E1[1. 实验设置]
E1 --> E11[数据集：Amazon 3子集（Instruments：1.7万用户，6.2千物品；Arts：2.2万用户，9.4千物品；Games：4.2万用户，1.3万物品）]
E1 --> E12[基线：传统序列（BERT4Rec、SASRec）、多模态（MISSRec、VIP5）、生成式（TIGER、MQL4GRec）]
E1 --> E13[指标：HR-1/5/10、NDCG-5/10]
E --> E2[2. 核心结果]
E2 --> E21[性能：Instruments数据集HR-10=0.1363、NDCG-10=0.1046（超MQL4GRec 4.9%）]
E2 --> E22[冲突率：文本ID冲突率降低0.38%-0.94%，图像ID降低0.42%-0.86%]
E2 --> E23[码本分配：物品在码本中分布更均匀，码本利用率提升15%-20%]

A --> F[研究价值]

F --> F1[理论：首次将跨模态对比学习融入RQ-VAE量化，突破单模态GR局限]
F --> F2[工程：支持工业级数据（Games数据集37万交互），推理采用约束束搜索+结果融合]
F --> F3[实践：降低冲突率、提升码本利用率，为多模态生成推荐提供新范式]
```
