...
## 研究背景与问题
- 传统LoRA持续学习局限
  - 单一演进LoRA：可塑性强但易遗忘有用历史知识
  - 累积LoRA（SumLoRA/SD-LoRA）：稳定性强但纠缠过时偏好，存储成本高
- 推荐持续学习特殊性
  - 目标：预测未来偏好而非过去，过时偏好会损害性能
  - 挑战：平衡“保留长期偏好”与“更新新兴趋势”
## PESO框架设计
- 1. 核心思路
  - 单一适配器：维持1个LoRA适配器，避免多适配器的任务独立性假设
  - 近邻正则化：锚定当前适配器与最新冻结状态，动态平衡稳定与塑性
- 2. 关键组件
  - 混合损失函数：交叉熵损失（数据拟合）+近邻项（稳定性约束）
  - softmax-KL近邻项：模块级正则，保护参数内部结构，局部等价于p加权方差
  - 理论保障：在LoRA子空间提供数据感知方向引导，强数据支持方向偏向新最优，弱支持方向保留历史
- 3. 训练配置
  - 基础模型：Llama-3.2 1B
  - 超参数：λ（正则强度）搜索[0.5,8.0]，学习率为预训练率的0.05-0.1倍，滑动窗口大小20
## 实验验证
- 实验设置
  - 数据集：Amazon 3个子集（Instruments：2.3万用户；Movies & TV：2.3万用户；Books：1.8万用户）
  - 基线：单一演进LoRA、累积LoRA家族（SumLoRA/SD-LoRA/InfLoRA）、传统持续推荐（PISA）
  - 指标：Hit@5/10、NDCG@5/10
- 关键结果
  - 性能：Books数据集Hit@10=0.0569（超SumLoRA latest+inherit 4.98%）
  - 正则对比：softmax-KL近邻项比L2正则提升2.17%（Movies & TV数据集）
  - 与传统模型对比：在Books数据集NDCG@10超PISA 16.6%
## 研究结论与价值
- 结论：单一演进LoRA+模块级近邻正则是推荐持续学习的最优范式
- 价值：1）解决过时偏好纠缠问题；2）低存储成本（仅1个适配器）；3）理论可解释，实践性能优
```
