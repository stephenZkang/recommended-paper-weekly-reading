...
## 研究背景：核心痛点
- 时间分布偏移（TDS）危害
  - 用户行为受内（大促）外（季节）因素影响，违反IID假设
  - 工业增量训练易丢失长期偏好，导致噪声更新
- 现有方案缺陷
  - 不变学习：环境划分难、存储/训练开销高（需混合历史数据）
  - 对比SSL（InfoNCE）：抑制个性化信号，引发表示坍缩
## 核心框架：ELBOₜₙₛ
- 1. 实证洞察与假设
  - 特征分析：统计特征CV=3-5、序列特征CV≈0.4、分类特征JSD累积上升
  - 核心假设：短期用户偏好由稳定因子s主导，时变因子v仅引发波动
- 2. 数据增强策略
  - 统计特征：分桶扰动（B'=B+ε，ε~U{-r,r}）
  - 序列特征：随机掩码（mask~Bern(p_seq)）
  - 分类特征：嵌入 dropout（p_cate=0.1-0.3）
  - 全局约束：随机选择特征子集扰动，避免超现实样本
- 3. ELBOₜₙₛ目标函数
  - 自监督组件：重构损失（MSE）+先验损失（多视图嵌入对齐）
  - 监督组件：预测损失（CTR/购买等任务损失）
  - 总损失：L_total = L_pred + α·(L_recon + L_prior)
- 4. 训练与部署
  - 训练：双塔架构，实时生成J=2-4个增强视图，联合优化
  - 部署：接入粗排+精排两阶段，无额外在线延迟
## 实验验证
- 数据集：Shopee-Main（300亿样本）、Shopee-Small（76亿样本）、KuaiRand-1K
- 基线：ERM、IRM、V-REx、SSL4Rec、Dino4Rec
- 关键结果：离线AUC+0.93%、GMV+2.33%、大促场景稳定性提升
## 核心价值
- 理论：首次结合因果变分与数据增强，为TDS提供统一框架
- 实践：适配工业增量 pipeline，部署Shopee搜索，支持亿级用户
- 数据：开源Shopee-Small与KuaiRand-1K，填补TDS工业数据集空白
```
