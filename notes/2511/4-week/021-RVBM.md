...
## 研究背景：核心痛点
- 回访行为建模难
  - 归因模糊：多混淆因素（通知/界面/意图变化）影响回访触发
  - 规模挑战：回访可延迟数周，需跨会话/跨表面追踪
- 现有方案局限
  - 聚焦短期指标（点击/保存），忽视长期留存
  - RL方法：难处理多日序列，缺乏Pinterest级规模适配
## 核心框架：回访建模多任务体系
- 1. 回访行为分析
  - 行为类型：印象式回访（高覆盖）、点击式回访（高意图）
  - 关键发现：3天内回访点击量衰减50%，点击式回访关联更高留存
- 2. 标签与特征设计
  - 回访标签：1dRevImpre（同日印象）、1dRevGrid（同日点击）、7dRevGrid（7天点击）
  - 回访特征：Related Pins触发特征（7/30/90天）、全局回访特征（覆盖率70%-80%）
- 3. 多任务模型集成
  - 基础架构：Transformer序列模块+DCNv2特征交互+MMoE多任务头
  - 新增回访头：损失函数=原损失+w·回访二分类损失，权重u=1.27×保存任务权重
- 4. 跨表面数据 pipeline
  - 数据关联：按用户ID+Pin ID关联保存（Related Pins）与回访（个人主页）
  - 流程：特征构造→标签提取→跨表面join→标签合并→特征-标签匹配
## 实验验证
- 数据集：66亿训练样本，7亿评估样本
- 关键结果：离线Hits@3回访+0.65%，在线活跃用户+0.1%
- 主题洞察：Beauty回访率高，Event Planning长期回访占比18.81%
## 核心价值
- 理论：首次大规模分析回访模式，建立保存-回访因果关联
- 实践：部署Related Pins，服务5亿用户，无额外成本
- 应用：优化长期留存，指导主题级推荐策略
```
