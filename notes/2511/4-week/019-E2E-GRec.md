...
## 研究背景：核心痛点
- 两阶段训练缺陷
  - 计算开销高：GNN需反复离线推理刷新嵌入（ billions级节点）
  - 缺乏联合优化：推荐模型梯度无法反传至GNN，目标错位
- 现有方案局限
  - 传统GNN部署：PinSage/LightGCN等仅离线生成嵌入，无梯度交互
  - 简单级联模型：梯度衰减严重，GNN受推荐信号指导弱
## 核心框架：E2E-GRec
- 1. 高效子图构建
  - 基础图：跨域异构图（CDG），含显式（用户行为）/隐式（i2i）边
  - i2i图构建：Swing算法（抑制活跃用户噪声，权重=1/(α+|Iu∩Iv|)）
  - 子图采样：importance sampling（概率∝ω^β，β控制浓度，k=100邻居/1-hop）
- 2. GNN多任务协同优化
  - 图特征自编码器（GFAE）：重构节点特征（MSE损失），提供稳定监督
  - 避免任务错位：SSL任务保留特征子空间，补充推荐任务梯度信号
- 3. 端到端联合训练
  - 两级特征融合：底部（门控拼接）+上部（多头注意力），增强特征交互
  - Gradnorm动态平衡：最小化梯度范数差异，防止任务主导（γ=1）
  - 总损失：L_total = w_SSL·L_SSL + w_LTR·L_LTR（权重自适应更新）
## 实验验证
- 数据集：TikTok生产数据（含用户行为/视频特征）
- 基线：传统LTR模型、级联GNN-LTR模型
- 关键结果：离线AUC+1.65%，在线停留时长+0.133%、Skip/User-0.3171%
## 核心价值
- 理论：证明端到端梯度耦合（Rec→GNN/GNN→Rec），解决目标错位
- 实践：适配工业大规模场景，部署TikTok推荐，低延迟高效率
- 应用：视频推荐、电商推荐等需GNN捕捉高阶关联的场景
```
