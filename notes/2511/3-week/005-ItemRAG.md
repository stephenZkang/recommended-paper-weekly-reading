...
## 研究背景：LLM推荐的RAG痛点
- 传统RAG局限
  - 用户级RAG：相似用户难识别（兴趣多样）、冷启动物品适配差
  - 零样本LLM推荐：缺乏共购模式感知，性能有限
- 核心矛盾
  - 用户兴趣难推断 vs 物品共购关系易捕捉
  - 冷启动物品共购数据少 vs 语义相似物品可提供间接信号
## ItemRAG框架设计
- 1. 核心思路
  - 物品级检索：从共购历史检索相关物品，而非相似用户
  - 双阶段解决检索挑战：语义相似补充（冷启动）+频率加权采样（相关性）
- 2. 关键组件
  - 语义相似物品筛选：Text Embedding（LM编码）→余弦相似度→Top-K（K=5）
  - 检索池构建：P(i)=N(i)∪{j|∃q∈T(i),j∈N(q)}（N(i)为i的共购物品，T(i)为相似物品）
  - 加权采样：权重w_ij=c_ij + (1/|T(i)|)∑c_qj，按权重采样N=50个物品
  - 摘要生成：LLM生成检索物品摘要，融入推荐提示
- 3. 推荐流程
  - 输入：用户购买历史+候选物品
  - 处理：对每个物品i检索→生成摘要→拼接原始描述→LLM排序
  - 输出：候选物品排序结果
## 实验验证
- 1. 实验设置
  - 数据集：Amazon 4个子集（Beauty/Toys/Sports/Arts）
  - 基线：传统模型（LightGCN、SASRec）、用户级RAG（CoRAL、AdaptRec）、零样本LLM
  - 指标：Hit@1/3/5、NDCG@3/5
- 2. 核心结果
  - 标准场景：Hit@1最高49.9（Beauty），超CoRAL 10%
  - 冷启动场景：性能仅下降1.3%，Hit@1最高47.6（Beauty）
  - 消融验证：无相似物品/无频率采样均导致性能下降
## 研究价值
- 理论：首次提出物品级RAG范式，突破用户级RAG局限
- 实践：冷启动鲁棒性强、可插件化集成现有LLM推荐
- 性能：全场景优于基线，4个数据集19/20指标第一
```
