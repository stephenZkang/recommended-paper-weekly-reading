...
```mindmap
## 研究背景：小米Home IoT操作推荐痛点
- 业务规模
  - 设备：260+子品类，2024年出货6000万台，累计9.44亿台
  - 用户：小米Home App月活1.13亿，2050万用户拥有≥5台设备
- 核心挑战
  - 复杂操作逻辑：需支持多设备组合操作+顺序依赖（如先开插座再开空调）
  - 用户偏好多样：长期习惯（固定时间开窗帘）+短期模式（避免频繁开关空调）
  - 劣质推荐敏感：不合理建议（如室温高时关空调）严重影响用户体验
## DevPiolt框架设计
- 1. 预训练：注入IoT领域知识
  - 数据：4.5万条/季度×3季度历史操作，混合1:1通用语料（ShareGPT/WuDao）
  - 任务：next-action预测，最小化负对数似然损失$`(\mathcal{L}_{pt})`$
  - 目标：让模型掌握设备功能（如灯光仅开关，空调可调温）与合法值范围
- 2. 微调：适配推荐任务
  - 数据：1.5万条手动标注样本（含操作提示、动作、描述）
  - 策略：Action-first（先生成动作四元组{房间,设备,字段,值}，再生成描述）
  - 优化：LoRA（rank=16, alpha=32）高效更新，多任务损失$`(\mathcal{L}_{ad})`$
- 3. 推荐优化：捕捉个性化偏好
  - DPO样本构建：9千条偏好对（正样本=用户实际操作，负样本=时间敏感/冲突操作）
  - DPO训练：优化$`(\mathcal{L}_{DPO})`$，提升正样本概率，降低负样本概率（β控制强度）
- 4. 曝光控制：过滤劣质推荐
  - 置信度计算：加权平均关键属性概率（设备/字段/值，权重$`(\alpha_k)`$）
  - 自适应策略：动态阈值（复杂任务降10%）、级联剪枝（低置信设备直接丢弃）、多维度融合（动作平均置信+首token置信）
## 实验验证
- 1. 实验设置
  - 数据集：9个细分集（4个设备数量集：1-5/6-10/11-20/20+；5个设备品类集：灯光/开关/摄像头/取暖器/空调），共4882条测试样本
  - 基线：CGC（特征基模型）、DeepSeek-V3、GPT-4o
  - 指标：EM-Acc（精确匹配准确率）、LM-F1（宽松匹配F1）、Rule（规则得分）
- 2. 核心结果
  - 全量数据集：EM-Acc=44.33%，LM-F1=57.49%，Rule=53.03%（超GPT-4o 32.6%/27.2%/25.3%）
  - 线上效果：UV覆盖率+21.6%，PV接受率+29.1%，日服务25.5万用户
## 研究价值
- 理论：提出LLM+DPO+曝光控制的IoT操作推荐范式
- 工程：LoRA轻量化更新（仅训练少量参数），支持实时推荐
- 实践：已落地小米Home App，每日生成32.6万条有效建议
```
