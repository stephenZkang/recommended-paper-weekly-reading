...
A --> C[研究背景：核心痛点]
C --> C1[传统序列推荐局限]
C1 --> C11[依赖ID嵌入，忽视跨类别潜在兴趣（如买帐篷未推荐睡袋）]
C1 --> C12[协同信号稀疏，长尾用户/物品推荐效果差]
C --> C2[LLM增强方案缺陷]
C2 --> C21[直接用生成内容作为监督，易受幻觉干扰（短序列用户幻觉率达4.9%）]
C2 --> C22[推理成本高，难以工业部署]

A --> D[核心框架：GRASP]

D --> D1[1. 生成增强检索]
D1 --> D11[生成：LLM按模板生成用户偏好/物品属性描述（支持Qwen2.5-7B等）]
D1 --> D12[编码：OpenAI API（公开数据集）/LLM2Vec（工业数据集）提取语义嵌入]
D1 --> D13[检索：余弦相似度匹配Top-k相似用户/物品，平均池化聚合特征]
D --> D2[2. 整体注意力增强]
D2 --> D21[注意力机制：Sigmoid替代Softmax，保留多兴趣]
D2 --> D22[多层融合：细粒度用户-物品注意力+全局相似特征注意力]
D2 --> D23[特征适配：MLP将增强特征映射为 backbone 输入维度]
D --> D3[3. 训练部署]
D3 --> D31[训练：冻结检索特征，仅优化注意力与backbone参数]
D3 --> D32[部署：离线预计算嵌入与相似检索，在线仅需注意力计算（复杂度O（l²d））]

A --> E[实验验证]

E --> E1[数据集：Amazon Beauty/Fashion、Industry-100K（9.9万用户+120万物品）]
E --> E2[基线：GRU4Rec、BERT4Rec、SASRec、LLM-ESR等]
E --> E3[关键结果]
E3 --> E31[离线：NDCG-10最高42.76%，长尾性能提升5%-10%]
E3 --> E32[在线：CTR+0.14pt，GMV+1.71%]


A --> F[研究价值]

F --> F1[理论：提出抗幻觉的LLM知识融合范式，不依赖监督信号]
F --> F2[实践：支持多backbone（GRU4Rec/BERT4Rec/SASRec），工业部署轻量化]
F --> F3[应用：覆盖电商、美妆等场景，提升长尾发现率与用户体验]
```
