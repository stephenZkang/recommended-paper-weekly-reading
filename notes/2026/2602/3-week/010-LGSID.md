...
C --> C1[现有方法局限]
C1 --> C11[文本基推荐：仅依赖LLM语义理解，缺乏地理感知（如推荐北京用户上海餐厅）]
C1 --> C12[ID基推荐：依赖协同信号，冷启动物品曝光不足，候选池扩大后性能瓶颈]
C --> C2[关键挑战]
C2 --> C21[细粒度空间捕捉：LLM难以区分文本相似但地理不同的位置（如安徽苏州vs江苏苏州）]
C2 --> C22[语义与地理平衡：预训练LLM优先内容相关性，忽视地理 proximity]
C2 --> C23[令牌质量不足：现有分词方法未融合地理信息，无法支撑本地生活场景]

A --> D[LGSID框架设计]

D --> D1[1. 基于RL的地理LLM对齐（注入地理知识）]
D1 --> D11[奖励模型训练]
D11 --> D111[数据构建：固定物品内容，替换位置生成提示序列（如P_i=（内容,真实位置）, P_i^j=（内容,负位置））]
D11 --> D112[模型结构：LLM编码提示→MLP预测奖励分，软标签基于哈弗斯距离分配]
D11 --> D113[损失函数：加权二元交叉熵损失（L_RM）]
D1 --> D12[G-DPO算法]
D12 --> D121[样本构建：D_mix=D_dc（领域协同对，共现得分>阈值）∪D_gc（地理约束对，随机采样异地物品）]
D12 --> D122[对齐损失：结合奖励模型得分，优化正/负样本概率比（β控制尖锐度）]
D12 --> D123[相似度正则：批内对比损失（L_sim），保留LLM语义表征]
D --> D2[2. 分层地理物品分词（生成地理感知令牌）]
D2 --> D21[主令牌生成：多维度特征加权拼接（行政编码f_admin、地理编码f_geo、品类f_cat、品牌f_brand）→MiniBatch K-Means聚类]
D2 --> D22[残差令牌优化：可学习聚类中心，通过欧氏距离分配令牌，最小化重建损失（L_recon）]
D2 --> D23[码本正则：熵正则（L_reg）避免码本坍缩，仅应用于残差层（l≥2）]


A --> E[实验验证]

E --> E1[1. 实验设置]
E1 --> E11[数据集：快手本地生活（5000万样本、1908万用户、232万物品、818品类）]
E1 --> E12[基线：判别式（DIN、DIEN）、生成式（TIGER、OneRec）、分词方法（RQ-VAE、Res-KMeans）]
E1 --> E13[指标：判别式（AUC）、生成式（Hit-5/10、NDCG-5/10）、地理感知（NMI、省市镇覆盖率）]
E --> E2[2. 核心结果]
E2 --> E21[判别式推荐：DIN+LGSID AUC=0.6276（超基线4.17%）]
E2 --> E22[生成式推荐：TIGER+LGSID Hit-10=0.5077（超基线30.83%）]
E2 --> E23[地理感知：NMI从0.0137→0.6430（省份）、0.0845→0.8644（城市）]

A --> F[研究价值]

F --> F1[理论：提出LLM+RL+分层分词的地理感知推荐范式，突破文本基方法局限]
F --> F2[工程：支持大规模候选池（232万物品），码本利用率均衡（熵正则控制）]
F --> F3[实践：已适配判别式/生成式推荐，为本地生活、外卖等场景提供解决方案]
```
